...
{
    /* APB2=64MHz. Initialization stayed at /32=2MHz; runtime tries
     * /4=16MHz and automatically backs off if readback is unreliable. */
...
/**
 * @brief  Bật bộ đếm chu kỳ DWT của Cortex-M3 làm đồng hồ µs.
 */
void MCU_TimerInit(void)
{
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;  /* cho phép DWT */
    DWT->CYCCNT = 0;
 ...
 *        BLOCKING ~1ms (có HAL_Delay). Chỉ gọi khi radio IDLE.
 */
